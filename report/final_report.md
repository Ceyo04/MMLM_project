...
本文构建了一个基于 Qwen2.5-VL-3B 视觉语言模型的智能图文问答助手，支持自然场景图片和文档/幻灯片截图两类图像的中文问答与多轮对话。系统采用 transformers 推理框架，使用 Gradio 构建 Web 交互界面，在 VQA-v2 验证集子集上进行了定量评测。在 50 条评测样本上取得 78% 的整体准确率，其中计数类问题表现突出（66.7%）。本文对错误类型进行了系统分类分析，并对模型的局限性和改进方向进行了讨论。
...
| 是否类（Yes/No） | **100.0%** | 17 |
| 计数类（How many...） | 66.7% | 9 |
| 其他（What/Where/Who...） | 66.7% | 24 |

**分析：** 通过强制英文短答案 Prompt 和标准 VQA 匹配策略，是否类问题准确率达到 100%，说明模型的视觉判断能力本身没有问题，之前的低分完全是由中文输出与英文标准答案的不匹配导致的。计数类和其他类问题准确率均为 66.7%，反映 3B 模型在细粒度视觉识别和推理方面的固有能力边界。

### 4.3 TextVQA 评测结果

在 TextVQA 验证集中随机抽取 100 条样本进行评测，结果如下：
...
**分析：** TextVQA 侧重图像中文字的识别与理解（如品牌名、路牌、数字等）。72% 的准确率说明 Qwen2.5-VL-3B 具备基本的 OCR 和文字推理能力。主要失败原因包括：小尺寸文字误读（如球衣号码）、模糊商标名识别错误、以及需要细粒度数值比较的场景。

### 4.4 VQA-v2 错误类型分布

对 VQA-v2 剩余 11 个错误样本进行分类后，分布如下：
...
**分析：** 修复评测管线后，剩余 11 个错误中绝大多数（72.7%）是推理/计数层面的真实模型缺陷，而非评测噪音。这说明经过 Prompt 工程和双语匹配优化后，3B 模型在基础视觉识别和 Yes/No 判断上已能胜任，但在需要精细空间推理、细粒度属性判断和外部知识的任务上仍存在不足。
...
本文基于 Qwen2.5-VL-3B 视觉语言模型构建了一个智能图文问答助手，实现了自然场景和文档/幻灯片两类图像的中文问答与多轮对话功能。系统在 VQA-v2 验证集子集（50 条样本）上取得了 78% 的整体准确率，在 TextVQA 验证集子集（100 条样本）上取得了 72% 的准确率，计数类问题准确率达到 66.7%。通过对 11 个错误样本的系统分类，发现 72.7% 源于推理/计数层面，准确的评测管线使模型的真实能力边界得以显现。本文的工作展示了在有限显存（8GB）条件下构建可用多模态应用的可行方案，为后续在更强大硬件上的改进提供了明确的路线图。
